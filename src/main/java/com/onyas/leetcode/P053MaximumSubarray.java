 ...
 * <p/>
 * 思路我们在别的动态规划题目中也很常用，以后我们称为”局部最优和全局最优解法“。
 ...
 * 那么第i+1步的表达式是：
 * local[i+1]=Math.max(A[i], local[i]+A[i])，就是局部最优是一定要包含当前元素，所以不然就是上一步的局部最优local[i]+当前元素A[i]（因为local[i]一定包含第i个元素，
 * 所以不违反条件），但是如果local[i]是负的，那么加上他就不如不需要的，所以不然就是直接用A[i]；
 * global[i+1]=Math.max(local[i+1],global[i])，有了当前一步的局部最优，那么全局最优就是当前的局部最优或者还是原来的全局最优
...
public class P053MaximumSubarray {
    ...
    public static void main(String[] args){
        System.out.println(new P053MaximumSubarray().maxSubArray(new int[]{-2,1,-3,4,-1,2,1,-5,4}));
    }
}
